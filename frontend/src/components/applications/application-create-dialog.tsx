    ...
    <DialogContent className="registry-editor-dialog application-create-dialog" finalFocus={false}>
      <form className="registry-editor application-create-form" onSubmit={(event) => void submit(event)}>
        <DialogHeader className="registry-editor-header">
          ...
          <DialogDescription>{accepted ? displayName : "APIM 订阅"}</DialogDescription>
        </DialogHeader>
        ...
        <div className="registry-editor-body application-create-body">
          {!accepted ? <>
            {blocker && <div className="application-create-unavailable" role="status"><b>当前环境尚未启用创建</b><span>{blocker}</span><Button type="button" variant="outline" size="sm" disabled={refreshing} onClick={onRefresh}><RefreshCw size={13} className={refreshing ? "spin" : undefined} />重新检查</Button></div>}
            ...
            <dl className="application-create-defaults">
              <div><dt>初始月额度</dt><dd>{defaults ? new Intl.NumberFormat(getIntlLocale()).format(defaults.monthly_token_limit) : "未同步"}{defaults && <span data-no-localize> Tokens</span>}</dd></div>
              <div><dt>初始速率</dt><dd>{defaults ? new Intl.NumberFormat(getIntlLocale()).format(defaults.tokens_per_minute) : "未同步"}{defaults && <span data-no-localize> TPM</span>}</dd></div>
            ...
            {!failed && <>
              ...
              <div className="application-key-value"><code aria-label="订阅密钥已隐藏">********************************</code><Button type="button" variant="outline" size="icon-sm" disabled={copying} onClick={() => void copyKey()} aria-label="复制订阅密钥" title="复制订阅密钥"><Copy size={14} /></Button></div>
            </>}
        ...
        <DialogFooter className="registry-editor-footer">
          <Button type="button" variant="outline" onClick={close} disabled={submitting || copying}>{accepted ? "关闭" : "取消"}</Button>
